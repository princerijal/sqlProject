/*
  # Fix query_history action in execute_query_with_audit

  1. Changes
    - Change query_history action from 'executed' to 'approved' (closest valid action)
    - Add clear note that this represents execution
    - Or skip query_history insertion since it's not designed for execution tracking
*/
...
BEGIN
  ...
  -- Note: query_history is not used for execution tracking as it doesn't have an 'executed' action
  -- Execution is tracked in audit_logs and the queries.executed_at/executed_by columns
